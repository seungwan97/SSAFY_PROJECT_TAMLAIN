...
def updateDataSet():
    ...
    visitedPlaceList = ratings.groupby(['scheduleId', 'day'])['jejuPlaceId'].apply(list).reset_index(name='visitedPlaceList')
    ...
    confidence = pd.merge(confidence, place[["jejuPlaceId", "categoryId", "subCategoryId"]], left_on='consequents', right_on='jejuPlaceId')
    ...
    place.to_pickle('place.pkl')
    ratings.to_pickle('ratings.pkl')
    confidence.to_pickle('confidence.pkl')
    file_name = os.path.expanduser('algo')
